...
A Python tool for screening and filtering protein structures generated by AlphaFold2 (AF2) integrated with AFig (Antibody Fragment in silico generation) for computational protein binder design.

## Overview

AFig_screener_online automates the process of comparing original and AF2-generated protein structures, calculating structural alignment scores (TM-scores), and filtering candidates based on interaction quality metrics. The tool is designed for iterative protein binder design workflows where multiple rounds of generation and screening are performed.
...
|----------|-------|------|----------|-------------|
| `--origin_pdb_dir` | `-ori` | str | Yes | Directory containing original PDB structures before AFig |
| `--generated_pdb_dir` | `-g` | str | Yes | Directory containing PDB structures generated by AFig/AF2 |
...
3. **Off-Target Filtering**: Removes structures with TM-score ≤ 0.95 (indicating significant structural deviation)
...
1. **TM-Score Threshold**: Structures with TM-score > 0.95 pass (maintains structural similarity)
2. **PAE Interaction**: Candidates with PAE interaction scores in the lowest 5% are selected (indicates strong predicted binding)
